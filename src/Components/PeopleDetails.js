...
const PeopleDetails = () => {
    ...
    useEffect(()=>{
        fetch(`
        https://api.themoviedb.org/3/person/${id}?api_key=${process.env.REACT_APP_API_KEY_TMDB}`)
        ...
        <div class="card-img-overlay h-75 bg-dark bg-opacity-75 m-5">
          <div className="row">
            ...
            <div className="col-6">
              ...
              <Link to="/personas"><button type="button" className="btn btn-outline-danger mt-5">Volver</button></Link>
            </div>
          </div>
        </div>
      </div>
